	...
	<center>
		...
		<?php
			while($row=$stmh->fetch(PDO::FETCH_ASSOC)){
				if($row[authority]==2) continue;
		?>
		<form action="maUpdatePro.php" method="post">
				<tr align="center">
					<input type="hidden" name="userid" value="<?php echo $row['id'];?>">
					<td><input type="text" size="10" value="<?=$row['name']?>" disabled></td>
					<td><input type="text" size="10" value="<?=$row['dept_id']?>" disabled></td>
					<td><input type="text" size="15" value="<?=$row['phone']?>" disabled></td>
					<td><input type="text" size="20" value="<?=$row['email']?>" disabled></td>
					<td><select name="authority" >
						<option value=0 <?php if($row['authority']==0) echo "selected";?>>학생</option>
						<option value=1 <?php if($row['authority']==1) echo "selected";?>>교수</option>";
					</select>
				</td>
